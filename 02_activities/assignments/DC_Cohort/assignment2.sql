...
SELECT 
product_name||', '||coalesce(product_size,' ')||'('||coalesce(product_qty_type,'unit')||')' AS manager_list
FROM product;
...
ORDER BY x.customer_id, x.market_date;
...
DROP TABLE IF EXISTS temp.sales_grouped_dates;
...
DROP TABLE IF EXISTS temp.sales_ranked;
CREATE TEMP TABLE sales_ranked AS
SELECT
    market_date,
    total_sales,
...
SELECT
    vpc.vendor_name,
    vpc.product_name,
    SUM(5 * vpc.original_price) AS imaginary_money
FROM (
    SELECT
        v.vendor_name,
        p.product_name,
